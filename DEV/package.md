...
`cpa` 默认添加 `--no-daemon`，明确使用内嵌 App Server，避免启动器注入的 `-c` 配置触发共享后台服务降级警告。已传入该选项时不重复添加；显式使用 `--remote` 或 `--remote=...` 时不自动添加，以保留远程连接用法。只检查 `--` 分隔符之前的选项。此行为不改变模型、reasoning 或实验上下文配置。
