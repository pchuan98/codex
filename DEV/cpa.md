...
currently sets this capability to `false` for `gpt-6-astra`, so experimental
mode alone does not activate it. Other model IDs are not automatically opted in.

Upstream PR #47625 removes the separate startup rejection when token-budget
history/notes is explicitly enabled, or enabled through model defaults, for a
model without experimental-context capability. This does not remove the
capability check from experimental-mode activation and does not establish CPA
server compatibility with the history/notes protocol.
...
- `codex-rs/codex-api/src/rate_limits.rs`: Uses the upstream parser and ordering unchanged; the SSE boundary owns default-quota selection.
